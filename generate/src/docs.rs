...
// Parts of this code are taken from the pest project https://github.com/pest-parser/pest/blob/d16266af2e8e1da3aa2625f71e6994c67d410aa9/generator/src/docs.rs
...
mod tests {
    ...
    #[test]
    fn test_doc_comment() {
        let pairs = match parser::parse(Rule::grammar_rules, include_str!("../tests/testfile.pest"))
        {
            Ok(pairs) => pairs,
        ...
        assert_eq!(expected.get("foo"), doc_comment.line_docs.get("foo"));
        assert_eq!(expected.get("bar"), doc_comment.line_docs.get("bar"));
        assert_eq!(expected.get("dar"), doc_comment.line_docs.get("dar"));
